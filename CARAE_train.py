...
#            out_decoding_sf=torch.softmax(out_decoding,dim=2)
        ...
        line_out="%d train loss: AE %6.3f cri %6.3f gen %6.3f pre_cla %6.3f pre_reg %6.3f" %(epoch,
                    running_loss_AE/Ntrain_batch,
                   running_loss_cri/Ntrain_batch,
                   running_loss_gen/Ntrain_batch,
                   running_loss_pre_cla/Ntrain_batch,
...
#            batch_p = np.cat([batch_p_reg, batch_p_cla])
            ...
            noise=torch.normal(mean=mean0,std=0).to(device)
            out_decoding = model.AE(batch_x,batch_l,batch_p, noise)
            out2 = out_decoding[:,:-1]
            _,out_num_AE= torch.max(out_decoding,2)
            loss_AE_test=criterion_AE(out2.reshape(-1,Nfea),batch_x2.reshape(-1)).data
...
#            out_decoding_sf=torch.softmax(out_decoding,dim=2)
            ...
            if Ncla>0:
                Prop_pre_cla=Prop_pre[:,0:Ncla]
                loss_pre_cla_test=criterion_Pre_cla(Prop_pre_cla, batch_p_cla).data
            ...
            if Nreg>0:
                Prop_pre_reg=Prop_pre[:,Ncla:]
                loss_pre_reg_test=criterion_Pre_reg(Prop_pre_reg, batch_p_reg).data
